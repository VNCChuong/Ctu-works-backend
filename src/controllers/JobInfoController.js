...
const createJobInfo = async (req, res) => {
    try {
        const { jobTitle, location, jobDescription, jobRequirements, jobType,
            minSalary, maxSalary, datePosted, jobLevel, jobIndustry } = req.body
        if (!jobTitle || !location || !jobDescription || !jobRequirements || !jobType
            || !minSalary || !maxSalary || !datePosted || !jobLevel || !jobIndustry) {
